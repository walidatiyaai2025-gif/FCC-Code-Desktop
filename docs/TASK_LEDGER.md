...
`CURRENT_PHASE = P03` and P03 remains `IN_PROGRESS`. `FCCD-P03-001` through `FCCD-P03-006` are CLOSED from exact candidate validation, normal canonical integration, exact-current-main non-regression CI, and integrated-task reconciliation; `FCCD-P03-007` remains `PENDING`.
...
Re-run the Worker Protocol claim map before selecting more work. If no Priority 1–4 recovery work exists, the earliest dependency-valid current-phase task is `FCCD-P03-007 — Migration/recovery tests`. Do not begin P04 until all P03 mandatory tasks are CLOSED and its exact-head phase gate is PASS. `VERIFIED_FINAL_COMPLETE` remains false.