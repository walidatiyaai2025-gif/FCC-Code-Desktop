...
`CURRENT_PHASE = P03` and P03 remains `IN_PROGRESS`. `FCCD-P03-001` through `FCCD-P03-007` are CLOSED from exact candidate validation, normal canonical integration, exact-current-main non-regression CI, and integrated-task reconciliation. All mandatory P03 task rows are therefore closed, but the P03 exit gate is still `NOT_RUN`.
...
Re-run the Worker Protocol claim map before selecting more work. If no Priority 1–4 recovery work exists, the next legal action is the dedicated P03 exact-head phase reconciliation/exit-gate closure. Do not begin P04 until P03 has canonical `PHASE_EXIT_GATE=PASS` with `evidence/phases/P03/CLOSURE.md`. `VERIFIED_FINAL_COMPLETE` remains false.