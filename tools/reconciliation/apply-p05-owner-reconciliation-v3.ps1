...
if ($LASTEXITCODE -ne 0) { throw "P05 v2 reconciliation applicator failed with exit code $LASTEXITCODE." }

# V2 intentionally owns the semantic transformation. Normalize its Markdown JSON
# fence with a single-quoted template so PowerShell cannot consume backticks.
...
Write-Host 'P05 v3 reconciliation applied with canonical owner-queue fencing.'
