...
if ($RunNegativeFixtures) {
    Assert-Rejected { Assert-Contract $root $policyText $queueText $ledgerText $phaseText ($controlText.Replace('KNOWN_RELEASE_BLOCKERS: 1','KNOWN_RELEASE_BLOCKERS: 0')) $targetText $finalText $false } 'project-control blocker drift'
    ...
    Assert-Rejected { Assert-Contract $root $policyText $queueText $ledgerText ($phaseText.Replace('DEFERRED_PHASE_GATES: P04=NOT_RUN','DEFERRED_PHASE_GATES: P04=NOT_RUN;P05=NOT_RUN')) $controlText $targetText $finalText $false } 'resolved P05 gate falsely deferred'
