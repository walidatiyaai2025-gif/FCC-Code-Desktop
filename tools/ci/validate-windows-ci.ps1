...
Write-Host 'Negative fixtures verified runner, SDK, permissions, locked restore, Release build, complete tests, build metadata, quality, FCC environment discovery, FCC runtime health/version compatibility, FCC structured runtime, FCC CLI fallback runtime, design-system, semantic-theme, app-chrome, workspace-layout, navigation-surface, bottom-tool-panel, command-palette, common-state, and DPI/resolution layout enforcement.'
