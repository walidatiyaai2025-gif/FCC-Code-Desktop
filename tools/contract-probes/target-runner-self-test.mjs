...
function git(cwd, args) {
  ...
  if (result.error || result.status !== 0) {
    throw new Error(`git ${args.join(' ')} failed: ${result.error ?? result.stderr.trim()}`);
  }
  return result.stdout;
}

function sourceDirtyEntries(cwd) {
  const result = run('git', [
    ...
    '--',
    '.',
    ':(exclude)evidence/phases/P00/target/**',
  ], { cwd });
  if (result.error || result.status !== 0) {
    throw new Error(`git status pathspec check failed: ${result.error ?? result.stderr.trim()}`);
  }
...
function runGitPathspecRegression() {
  ...
  try {
    ...
    git(temp, ['add', '.']);
    git(temp, ['commit', '--quiet', '-m', 'fixture baseline']);
    ...
    let dirty = sourceDirtyEntries(temp);
    assert(dirty.some((line) => line.includes('evidence/phases/P00/unity/historical.md')), 'Non-target evidence changes must remain blocking.');
    git(temp, ['checkout', '--quiet', '--', 'evidence/phases/P00/unity/historical.md']);

    write(temp, 'tools/contract-probes/run-target-validation.ps1', '# changed executable input\n');
    dirty = sourceDirtyEntries(temp);
    assert(dirty.some((line) => line.includes('tools/contract-probes/run-target-validation.ps1')), 'Tracked executable-input changes must remain blocking.');
    git(temp, ['checkout', '--quiet', '--', 'tools/contract-probes/run-target-validation.ps1']);

    write(temp, 'tools/contract-probes/untracked-probe.mjs', 'console.log("untracked");\n');
    dirty = sourceDirtyEntries(temp);
    assert(dirty.some((line) => line.includes('tools/contract-probes/untracked-probe.mjs')), 'Untracked executable-input files must remain blocking.');
    fs.rmSync(path.join(temp, 'tools/contract-probes/untracked-probe.mjs'));

    write(temp, 'docs/new config with spaces.txt', 'dirty\n');
    dirty = sourceDirtyEntries(temp);
    assert(dirty.some((line) => line.includes('docs/new config with spaces.txt')), 'Space-containing source/config paths must remain blocking.');
    fs.rmSync(path.join(temp, 'docs/new config with spaces.txt'));
...
try {
  ...
  console.error(`TARGET_RUNNER_SELF_TEST_FAIL: ${error?.stack ?? error}`);
