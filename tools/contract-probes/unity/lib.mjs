...
export function writeDisposableFixtureSources(projectRoot) {
  ...
  const asmdef = (name) => JSON.stringify({ name, references: [], optionalUnityReferences: ['TestAssemblies'], includePlatforms: [], excludePlatforms: [], allowUnsafeCode: false, overrideReferences: false, precompiledReferences: [], autoReferenced: true, defineConstraints: [], versionDefines: [], noEngineReferences: false }, null, 2) + '\n';
  fs.writeFileSync(path.join(editModeDir, 'FccProbe.EditMode.asmdef'), asmdef('FccProbe.EditMode'), 'utf8');
