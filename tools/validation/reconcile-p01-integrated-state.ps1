...
function Replace-Exact {
    ...
    if (-not $Text.Contains($Old)) {
        throw "Expected reconciliation source not found: $Label"
    }
...
$phase = Replace-Exact $phase '- `P01` — IN_PROGRESS as the sole current phase; `FCCD-P01-001` through `FCCD-P01-006` remain PENDING at the transition boundary.' "- `P01` — IN_PROGRESS as the sole current phase; `FCCD-P01-001` through `FCCD-P01-005` are CLOSED and `FCCD-P01-006` remains PENDING.`n- P01 integrated-task reconciliation evidence: `evidence/phases/P01/INTEGRATED_TASK_RECONCILIATION_2026-09-03.md`." 'CURRENT_PHASE current status'
...
foreach ($task in @('001','002','003','004','005')) {
    $pattern = "(?m)^(\| FCCD-P01-$task \|[^\r\n]*\|) PENDING \|$"
    if ($ledger -notmatch $pattern) {
        throw "Expected P01-$task PENDING row not found."
    }
    $ledger = [regex]::Replace($ledger, $pattern, '$1 CLOSED |', 1)
}

$marker = '| FCCD-P01-006 | Build metadata/version service | PENDING |'
if (-not $ledger.Contains($marker)) {
    throw 'P01-006 must remain PENDING during integrated-task reconciliation.'
}
$ledger = $ledger.Replace($marker, $marker + "`n`nClosure evidence for `FCCD-P01-001` through `FCCD-P01-005` is recorded in `evidence/phases/P01/INTEGRATED_TASK_RECONCILIATION_2026-09-03.md`. `FCCD-P01-006` remains the only unresolved mandatory P01 task; P01 itself remains IN_PROGRESS and its exit gate has not been run.")

$currentNext = [regex]::Match($ledger, '(?ms)^## Current next action\s+.*\z')
if (-not $currentNext.Success) {
    throw 'Could not locate ledger Current next action block.'
}
if ($currentNext.Value -notmatch 'CURRENT_PHASE = P00') {
    throw 'Ledger Current next action no longer matches the stale P00 checkpoint expected by this reconciliation.'
}
$newNext = @'
## Current next action
...
| `FCCD-P01-002` | PR #45 normal merge `0b84859edaccc4a9bb9c407b68a8c4b842d436d1` | Windows/.NET `10.0.400` validation run `33670390900`: restore, format, Release build, nullable/analyzer/style negative fixtures, recovery and clean-worktree checks | CLOSED |
...
| `FCCD-P01-005` | PR #48 normal merge `99748c2fabf02c4c27c075e4ace1ebb7f1fc8d46`; follow-up PR #49 normal merge `9d3098e251d237752542a4602e4014a8fa1eebc9` | PR #48 exact-head Windows CI `33716877372` SUCCESS; main run `33717031238` SUCCESS; PR #49 exact-head Windows CI `33717169974` SUCCESS; main run `33717649592` SUCCESS | CLOSED |
...
Release acceptance rows remain governed by `docs/ACCEPTANCE_MATRIX.md`; this P01 task reconciliation does not convert release-level `NOT_RUN` rows into PASS.
'@
Set-Content -LiteralPath $evidencePath -Value $evidence.TrimEnd() -Encoding utf8NoBOM

foreach ($path in @($phasePath, $ledgerPath, $evidencePath)) {
    if (-not (Test-Path -LiteralPath $path)) { throw "Reconciliation output missing: $path" }
}

if ((Get-Content -LiteralPath $phasePath -Raw) -notmatch 'FCCD-P01-006.*remaining mandatory P01 implementation task') {
    throw 'CURRENT_PHASE reconciliation assertion failed.'
}
$finalLedger = Get-Content -LiteralPath $ledgerPath -Raw
foreach ($task in @('001','002','003','004','005')) {
    if ($finalLedger -notmatch "(?m)^\| FCCD-P01-$task \|.*\| CLOSED \|$") {
        throw "Ledger closure assertion failed for P01-$task."
    }
}
if ($finalLedger -notmatch '(?m)^\| FCCD-P01-006 \|.*\| PENDING \|$') {
    throw 'Ledger P01-006 PENDING assertion failed.'
}
if ($finalLedger -match 'CURRENT_PHASE = P00') {
    throw 'Stale P00 next-action checkpoint remains in ledger.'
}
