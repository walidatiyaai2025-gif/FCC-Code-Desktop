...
- `FCCD-P02-001` through `FCCD-P02-005` — CLOSED from validated canonical integration and exact-current-main non-regression CI.
- `FCCD-P02-006` through `FCCD-P02-009` — PENDING.
...
Apply `docs/WORKER_PROTOCOL.md` within P02. `FCCD-P02-001` through `FCCD-P02-005` are reconciled CLOSED. Reconcile any new legitimate active/recovery work first; otherwise the earliest dependency-valid task is `FCCD-P02-006 — Bottom tool panel framework`. Do not begin P03 until all mandatory P02 tasks are CLOSED, the P02 exit gate passes with exact-head evidence, `evidence/phases/P02/CLOSURE.md` is integrated, and canonical `main` is green. `VERIFIED_FINAL_COMPLETE` remains false.
...
8. Treat P02 as the sole legal implementation phase; `FCCD-P02-001` through `FCCD-P02-005` are CLOSED and `FCCD-P02-006` through `FCCD-P02-009` remain PENDING unless newer live repository state truthfully changes them.
