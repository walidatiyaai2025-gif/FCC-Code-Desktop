...
public sealed class SqliteDatabaseInitializerTests
{
    ...
    public async Task InitializeCreatesUnicodePathDatabaseAndAppliesBaselineMigrations()
    {
        ...
        Assert.Equal(2, result.AppliedVersions.Count);
    ...
    public async Task InitializeIsIdempotentAndDoesNotReapplyCompletedMigrations()
    {
        ...
        Assert.Equal(2, first.AppliedVersions.Count);
    ...
    public async Task FailedMigrationRollsBackAndCorrectedMigrationCanRecover()
    {
        ...
        Assert.Contains("migration 3", failure.Message, StringComparison.OrdinalIgnoreCase);
        ...
        Assert.Equal(3, Assert.Single(recovered.AppliedVersions));
        Assert.Equal(3, recovered.CurrentVersion);
    ...
    public void MigrationPlanRejectsVersionGapsBeforeTouchingDisk()
    {
        ...
        var databasePath = workspace.GetPath("state.db");
        var invalidMigration = new SqliteMigration(4, "skipped_version", "SELECT 1;");

        var failure = Assert.Throws<ArgumentException>(() =>
            new SqliteDatabaseInitializer(new SqliteDatabaseOptions(databasePath), [invalidMigration]));

        Assert.Contains("Expected version 3", failure.Message, StringComparison.Ordinal);
